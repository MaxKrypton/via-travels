...
// ---------------------------------------------------------------------------
// Shared pricing shapes — move these into utils/types.ts alongside the rest
// of your shared types once you're happy with the structure.
// ---------------------------------------------------------------------------

export type ResidencyAudience =
  | 'rwandan_citizen'                    // RDB wording (parks/permits)
  | 'national'                           // RCHA wording (museums) — same group
  | 'east_african_citizen'
  | 'eac_cepgl_citizen'                  // RCHA wording — same group as above
  | 'east_african_foreign_resident'
  | 'foreign_resident_in_rwanda'
  | 'international_resident_in_rwanda'   // RCHA wording — same group as above
  | 'international_non_resident'         // RCHA wording
  | 'foreigner';                         // RDB wording — same group as above

export interface ResidencyTier {
  audience: ResidencyAudience;
  adult: number | null;
  childOrStudent: number | null;
  currency: 'USD' | 'RWF';
}
...
export interface TieredResidencyPricing {
  ...
  tiers: ResidencyTier[];
  groupRates?: Record<string, unknown>;
...
export interface DistanceFormulaPricing {
  pricingModel: 'distance_formula';
  currency: 'RWF';
  baseFare: { upToKm: number; amount: number };
  perAdditionalKm: number;
  waiting: { freeMinutes: number; perMinuteAfter: number };
  source: string;
  sourceDate: string;
}
...
const RCHA_BOOKING_URL =
  'https://irembo.gov.rw/user/citizen/service/rcha/visit_schedule_for_national_museums_of_rwanda';

function rchaEntranceFee(specialExhibitions?: Record<string, unknown>): TieredResidencyPricing {
  return {
    pricingModel: 'tiered_by_residency',
    currency: 'RWF',
    tiers: [
    ...
    notes:
      'Single ticket valid 3 hours. Combined ticket (2+ museums) valid 3 months — ' +
...
export const tourismEntriesBatch2 = [
  // 1. Golden monkey trekking permit ----------------------------------------
  {
    category: 'permit' as const,
    name: 'Golden Monkey Trekking Permit',
    description:
      'Official RDB permit required to trek golden monkeys in Volcanoes National Park. ' +
    ...
    pricingDetails: {
      ...
      notes:
        'No published child/student rate for the "foreigner" tier (minimum trekking age is 12 ' +
  ...
  // 2. Kings' Palace Museum — Nyanza -----------------------------------------
  {
    ...
    description:
      'Reconstructed royal residence and cattle kraal in Nyanza, former capital of the ' +
  ...
  // 3. Kandt House Museum -----------------------------------------------------
  {
    ...
    description:
      'Former residence of Richard Kandt, the first European colonial administrator of ' +
  ...
  // 4. Rwanda Art Museum (Kigali) ----------------------------------------------
  {
    ...
    description:
      'Former presidential palace in Kigali, now a contemporary and traditional art museum. ' +
  ...
  // 5. Rwanda Liberation Museum -------------------------------------------------
  {
    ...
    description:
      'National museum dedicated to the RPA/RPF liberation struggle. Exact site location ' +
  ...
  // 6. Ethnographic Museum (Huye) -----------------------------------------------
  {
    ...
    description:
      "Rwanda's main national ethnographic museum, covering traditional culture and daily " +
  ...
  // 7. Museum for Campaign Against Genocide (Kigali) ----------------------------
  {
    ...
    description:
      "Kigali museum (Camp Kigali area) documenting the RPA's campaign to stop the 1994 " +
  ...
  // 8. Moto-taxi (Kigali) ---------------------------------------------------
  {
    ...
    description:
      'RURA-regulated moto-taxi fare formula for trips within Kigali. This is a distance-based ' +
    ...
    pricingDetails: {
      ...
      sourceDate: '2024',
    } satisfies DistanceFormulaPricing,
...
export async function seedBatch2() {
  const inserted = await database.insert(tourismEntries).values(tourismEntriesBatch2).returning();
